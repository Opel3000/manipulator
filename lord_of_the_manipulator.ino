...
void search_spiers(byte outputMas[3][3]) {
  ...
  for (byte i = 0; i <= 2; i++) {
    for (byte j = 0; j <= 2; j++) {
      if (outputMas[i][j] > 5){
        if (countSpiersNice == 0) indexDom = j;
